...
## 2026-04-09 - [SQL Injection Risk in Admin Order Sorting]
**Vulnerability:** The `AdminService.getAllOrders` method directly interpolated the `sortBy` query parameter into a TypeORM `orderBy` clause, allowing potential SQL injection.
**Learning:** While TypeORM handles parameterization for values, identifiers like column names in `orderBy` are often treated as raw strings if not carefully handled, creating injection vectors when exposed to user input.
**Prevention:** Always whitelist allowed sort fields and sort orders in service methods. Validate that input matches expected enum-like values before passing them to query builders.
