...
## 2026-03-10 - [Price Manipulation in Payments]
**Vulnerability:** The `POST /api/v1/payments/create-intent` endpoint accepted an `amount` from the client and used it for Stripe PaymentIntent creation without verifying it against the database.
**Learning:** Client-provided values should never be trusted for financial transactions. Even if the frontend calculates the price, the backend must re-verify or retrieve the price from a trusted source (database).
**Prevention:** Always retrieve order totals and product prices from the database server-side when initiating payments. Ignore client-provided amounts for transactions.
