...
## 2026-03-10 - [Price Manipulation in Payment Intents]
**Vulnerability:** The `POST /api/v1/payments/create-intent` endpoint accepted an `amount` from the request body, allowing a user to pay an arbitrary amount for any order.
**Learning:** Never trust client-provided values for financial transactions. Even if the UI shows the correct price, a malicious user can intercept and modify the request.
**Prevention:** Always fetch the transaction amount from the database (source of truth) using a trusted identifier (e.g., `orderId`) and verify resource ownership before initiating a payment.
