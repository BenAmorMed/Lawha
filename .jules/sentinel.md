...
## 2026-05-31 - [SQL Injection in OrderBy Clauses]
**Vulnerability:** The `AdminService.getAllOrders` method directly interpolated the `sortBy` query parameter into a TypeORM `.orderBy()` call, allowing potential SQL injection or at least unexpected database behavior.
**Learning:** Even when using an ORM like TypeORM, certain methods (like `orderBy` or raw queries) may not automatically parameterize or escape all inputs if used with template strings.
**Prevention:** Always whitelist allowed sort fields and directions. Use a safe default and strictly validate that the input matches the whitelist before passing it to the database query builder.
