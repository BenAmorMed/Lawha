...
## 2026-05-15 - [SQL Injection in Admin Sort Logic]
**Vulnerability:** The `AdminService.getAllOrders` method used unsanitized user input from query parameters directly in a TypeORM QueryBuilder `orderBy()` call.
**Learning:** TypeORM `orderBy()` does not automatically parameterize or escape column names, making it vulnerable to SQL injection if user-provided strings are interpolated.
**Prevention:** Always use a strict whitelist of allowed column names and sort directions for dynamic ordering logic.
