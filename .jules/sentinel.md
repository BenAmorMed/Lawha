...
## 2026-05-10 - [SQL Injection in Order Sorting]
**Vulnerability:** The `AdminService.getAllOrders` method used direct string interpolation of user-provided `sortBy` and `sortOrder` query parameters in a TypeORM QueryBuilder `orderBy()` call.
**Learning:** TypeORM's `orderBy()` method does not automatically parameterize or escape column names, making it a common vector for SQL injection if used with unsanitized user input.
**Prevention:** Always whitelist allowed column names for sorting and validate the sort direction (ASC/DESC) before passing them to database queries.
