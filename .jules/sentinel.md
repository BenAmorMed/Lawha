...
## 2026-06-05 - [SQL Injection in Dynamic Sorting]
**Vulnerability:** The `AdminService.getAllOrders` method used unvalidated query parameters directly in a TypeORM `orderBy` clause, allowing potential SQL injection.
**Learning:** TypeORM's `orderBy` method does not parameterize column names or sort directions, making it a common sink for injection if input whitelisting is skipped.
**Prevention:** Always implement a strict whitelist for `sortBy` fields and validate `sortOrder` against 'ASC'/'DESC' before passing them to the database query builder.
