...
## 2026-06-08 - [SQL Injection in Admin Sorting]
**Vulnerability:** The `AdminService.getAllOrders` method directly injected `sortBy` and `sortOrder` query parameters into a TypeORM `.orderBy()` clause using template strings, allowing for potential SQL injection.
**Learning:** Even when using an ORM like TypeORM, dynamic column names in clauses like `orderBy` are not automatically sanitized and must be strictly whitelisted.
**Prevention:** Always implement a strict whitelist of allowed column names for dynamic sorting and validate the sort direction (ASC/DESC). Default to a safe column and direction if input is invalid.
