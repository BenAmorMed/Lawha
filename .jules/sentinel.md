...
## 2026-03-10 - [SQL Injection in Admin Sorting]
**Vulnerability:** The `AdminService.getAllOrders` endpoint used string interpolation for the `orderBy` clause with unvalidated query parameters (`sortBy`, `sortOrder`), allowing potential SQL injection.
**Learning:** TypeORM's `orderBy` method (and similar query builder methods) can be vulnerable to injection if the column name or sort direction is derived directly from user input without strict whitelisting.
**Prevention:** Always implement a whitelist for dynamic sorting and filtering fields. Use `getManyAndCount()` to optimize performance and ensure that only allowed columns and directions are passed to the query builder.
