...
## 2026-07-03 - [SQL Injection in Dynamic Ordering]
**Vulnerability:** The `AdminService.getAllOrders` method used unsanitized user input directly in a TypeORM `.orderBy()` clause, potentially allowing SQL injection.
**Learning:** TypeORM's `QueryBuilder.orderBy()` does not automatically parameterize the column name, making it vulnerable if user-provided strings are interpolated directly.
**Prevention:** Always implement a strict whitelist for `sortBy` and `sortOrder` parameters before passing them to `orderBy()`. Use a default safe value if the input does not match the whitelist.
