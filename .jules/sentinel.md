...
## 2026-06-11 - [SQL Injection in Admin Orders Query]
**Vulnerability:** The `AdminService.getAllOrders` method used unsanitized user input from `sortBy` and `sortOrder` query parameters directly in a TypeORM `.orderBy()` call, allowing potential SQL injection.
**Learning:** TypeORM's `.orderBy()` does not automatically parameterize or escape column names when passed as string templates. Runtime validation/whitelisting is required even if TypeScript types suggest a set of allowed strings.
**Prevention:** Always implement a strict whitelist of allowed sort columns and validate sort directions (ASC/DESC) before passing them to the database layer.
