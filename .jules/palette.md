...
## 2025-05-20 - [Accessible Rating Components]
**Learning:** Rating components with multiple star icons can be noisy for screen readers if each icon is announced. Using a single container with `role="img"` and a calculated `aria-label` while hiding the individual icons via `aria-hidden="true"` provides a much cleaner experience. Additionally, supporting half-stars is crucial for accurate feedback.
**Action:** Implement rating components using a single accessible image role with a composite label and hide decorative internal icons.
