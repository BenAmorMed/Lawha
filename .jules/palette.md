...
## 2026-05-02 - [Rating Component Accessibility]
**Learning:** Standard star rating displays often announce as a series of redundant "star" icons or ambiguous numbers to screen readers. Grouping them into a single `role="img"` with a composite `aria-label` provides a much clearer summary of the product's social proof.
**Action:** Use `role="img"` and a descriptive `aria-label` for aggregate rating displays, while marking individual stars and raw counts as `aria-hidden="true"` to prevent redundant announcements.
