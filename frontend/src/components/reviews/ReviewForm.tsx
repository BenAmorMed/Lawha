  ...
  if (!user) {
    return (
      <div className="bg-secondary border border-gray-200 rounded-lg p-6 text-center">
        ...
        <Link
          href="/login"
          className="inline-flex items-center justify-center font-semibold transition-colors duration-200 rounded-md bg-primary text-white hover:bg-primary-dark active:bg-primary-dark px-6 py-2.5 text-sm"
        >
          Sign In
        </Link>
      </div>
        ...
        <div className="flex gap-2">
          {[1, 2, 3, 4, 5].map((star) => (
            <button
              ...
              className={`text-3xl transition hover:scale-110 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary rounded`}
            >
              <span className={star <= rating ? 'text-star' : 'text-gray-300'}>
                ★
              </span>
            </button>
          ))}
