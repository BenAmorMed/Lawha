...
export default function ReviewForm({
  ...
  if (!user) {
    return (
      <div className="bg-pink-50 border border-primary/20 rounded-lg p-6 text-center">
        <p className="text-primary-dark font-medium mb-4">Sign in to leave a review</p>
        <Link
          href="/login"
          className="inline-block px-6 py-2 bg-primary text-white rounded-lg hover:bg-primary-dark transition-colors font-bold shadow-sm"
        >
          Sign In
        </Link>
      </div>
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-lg p-6 border border-gray-200">
        ...
        <div className="flex gap-2" role="radiogroup" aria-label="Star rating">
          {[1, 2, 3, 4, 5].map((star) => (
            <button
              ...
              className={`text-3xl transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 rounded ${
                star <= rating ? 'text-star' : 'text-gray-300 hover:text-star/50'
              }`}
            >
              ★
            </button>
      ...
      <button
        ...
        className="w-full px-6 py-2 bg-primary text-white rounded-lg hover:bg-primary-dark disabled:bg-gray-300 font-bold transition-colors shadow-sm"
      >
        {loading ? (
