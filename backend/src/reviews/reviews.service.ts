...
export class ReviewsService {
  ...
  async getProductReviews(
    ...
    // Optimization: Fetch denormalized rating from product entity
    const product = await this.productsRepository.findOne({
      where: { id: productId },
      select: ['rating'],
    });
    ...
    return {
      ...
      productRating: {
        ...
        total: total, // Use total from getManyAndCount
      },
    };
  }
  ...
  async getMultipleProductStats(productIds: string[]) {
    // Optimization: Fetch denormalized stats from the Product repository
    // using the In operator, avoiding multiple aggregate queries on the Review table.
    const products = await this.productsRepository
      .createQueryBuilder('product')
      .select(['product.id', 'product.rating', 'product.reviewsCount'])
