...
export class AdminService {
  ...
  async getOrderAnalytics() {
    // Optimization: Consolidate 6 individual database queries into 2 to reduce database roundtrips by 66%.
    // Query 1: Get status-based distribution and aggregate totals in one go.
    ...
    // Query 2: Get last 7 days distribution.
    ...
    // Calculate summary metrics in-memory from query results to avoid redundant DB calls.
    let totalOrders = 0;
    let totalRevenue = 0;
    let grandTotalSum = 0;
    ...
    return {
      summary: {
        ...
        average_order_value: totalOrders > 0 ? grandTotalSum / totalOrders : 0,
