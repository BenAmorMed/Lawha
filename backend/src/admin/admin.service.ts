...
export class AdminService {
  ...
  async getAllOrders(filters: {
    ...
    // Optimization: Use getManyAndCount() to retrieve both data and total in a single database roundtrip.
    // Security: Whitelist sortBy fields to prevent SQL injection.
  ...
  async getOrderAnalytics() {
    ...
    // Query 1: Get all status-based metrics in a single group-by query.
    ...
    // In-memory summary calculations
    let totalOrders = 0;
    let totalRevenue = 0;
    ...
    statusMetrics.forEach((item) => {
      ...
      totalOrders += count;
      totalValueAllOrders += sum;
      statusBreakdown[status] = count;

      // Revenue only from completed (shipped/delivered) orders
    ...
    return {
      summary: {
        ...
        average_order_value: totalOrders > 0 ? totalValueAllOrders / totalOrders : 0,
