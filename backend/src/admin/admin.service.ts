...
export class AdminService {
  ...
  async getOrderAnalytics() {
    // Optimization: Consolidate 6 separate queries into 2 grouped queries to reduce DB roundtrips.
    // Query 1: Get status breakdown, total count, revenue, and average in a single GROUP BY.
    const statsByStatus = await this.ordersRepository
      .createQueryBuilder('order')
      .select('order.status', 'status')
      .addSelect('COUNT(order.id)', 'count')
      .addSelect('SUM(order.total)', 'sum')
      .addSelect('AVG(order.total)', 'avg')
      .groupBy('order.status')
      .getRawMany();

    // Query 2: Get daily distribution for the last 7 days.
    ...
    // Calculate summary metrics in-memory from the results
    let totalOrders = 0;
    let revenue = 0;
    let totalPoints = 0;
    ...
    statsByStatus.forEach((item) => {
      ...
      // Revenue: total amount from 'shipped' or 'delivered' orders
      if (status === 'shipped' || status === 'delivered') {
        revenue += parseFloat(item.sum || 0);
      }

      totalPoints += parseFloat(item.sum || 0);
    });
    ...
    return {
      summary: {
        ...
        average_order_value:
          totalOrders > 0 ? parseFloat((totalPoints / totalOrders).toFixed(2)) : 0,
