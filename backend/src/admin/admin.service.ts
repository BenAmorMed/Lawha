...
export class AdminService {
  ...
  async getOrderAnalytics() {
    // Optimization: Consolidate multiple aggregate queries into two database roundtrips.
    // Query 1: Get status-based aggregates (count, total, avg)
    ...
    // Derived summary metrics calculated in-memory to save 3 database roundtrips
    ...
    // Recent orders range (last 7 days)
    ...
    // Derive recent orders count from ordersByDay in-memory to save 1 database roundtrip
    const recentOrdersTotal = ordersByDay.reduce(
    ...
    return {
      summary: {
        ...
        average_order_value:
          totalOrdersCount > 0 ? overallSum / totalOrdersCount : 0,
        orders_last_7_days: recentOrdersTotal,
      },
