...
export class AdminService {
  ...
  async getOrderAnalytics() {
    // Optimization: Consolidate multiple aggregate queries into status-grouped and date-grouped result sets.
    // Reduces database roundtrips from 6 to 2.

    // 1. Status-grouped aggregates (total orders, revenue, status breakdown, avg order value)
    ...
    statusStats.forEach((item) => {
      ...
      // Revenue only includes shipped or delivered orders
      if (['shipped', 'delivered'].includes(item.status)) {
        totalRevenue += sum;
      }
    });

    const averageOrderValue = totalOrders > 0 ? overallSum / totalOrders : 0;

    // 2. Date-grouped aggregates for the last 7 days
