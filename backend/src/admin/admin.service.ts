...
export class AdminService {
  ...
  async getOrderAnalytics() {
    ...
    // Optimization: Consolidate 6 sequential queries into 3 parallel branches
    const [statusBreakdown, revenueStats, ordersTrend] = await Promise.all([
      // Branch 1: Status breakdown (total orders will be derived in-memory)
      ...
      // Branch 2: Revenue and Global Average Value in one query
      this.ordersRepository
        .createQueryBuilder('order')
        .select(
      ...
      // Branch 3: Daily trend for last 7 days (7-day total will be derived in-memory)
      this.ordersRepository
        .createQueryBuilder('order')
    ...
    // Derive totals in-memory to save extra COUNT queries
    const totalOrders = statusBreakdown.reduce(
      (sum, item) => sum + parseInt(item.count, 10),
      0,
    );
    const ordersLast7Days = ordersTrend.reduce(
      (sum, item) => sum + parseInt(item.count, 10),
      0,
    );
    ...
    return {
      summary: {
        ...
        average_order_value: parseFloat(revenueStats?.average || 0),
      ...
      orders_by_day: ordersTrend.map((item) => ({
        date: item.date,
