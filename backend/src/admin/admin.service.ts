...
export class AdminService {
  ...
  async getOrderAnalytics() {
    // Optimization: Consolidate multiple queries into 2 queries.
    // Query 1: Get total count and sum of totals grouped by status.
    ...
    // Query 2: Orders by day (last 7 days).
    ...
    return {
      summary: {
        ...
        average_order_value:
          totalOrders > 0
            ? parseFloat((totalOverallAmount / totalOrders).toFixed(2))
            : 0,
        orders_last_7_days: ordersLast7Days,
      },
      status_breakdown: statusBreakdown,
      orders_by_day: ordersByDayFormatted,
    };
  }
