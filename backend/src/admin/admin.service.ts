...
export class AdminService {
  ...
  async getAllOrders(filters: {
    ...
    // Optimization: Use getManyAndCount to reduce DB roundtrips and loadRelationCountAndMap
    // to fetch item counts without fully hydrating the items collection.
  ...
  async getOrderAnalytics() {
    // Optimization: Consolidate multiple queries into two grouped queries to reduce DB roundtrips.
    // 1. Get status breakdown, total revenue, and average order value in one query.
    const statusStats = await this.ordersRepository
      .createQueryBuilder('order')
      .select('order.status', 'status')
      .addSelect('COUNT(order.id)', 'count')
      .addSelect('SUM(order.total)', 'sumTotal')
      .addSelect('AVG(order.total)', 'avgTotal')
    ...
    statusStats.forEach((item) => {
      ...
      const sum = parseFloat(item.sumTotal || 0);
      const status = item.status;

      totalOrders += count;
      statusBreakdown[status] = count;

      // Revenue only counts for completed orders (shipped/delivered)
      if (['shipped', 'delivered'].includes(status)) {
        totalRevenue += sum;
      }

      totalPointsForAvg += sum;
    });

    // 2. Get daily orders for the last 7 days.
    ...
    const ordersLast7Days = ordersByDay.reduce(
    ...
    return {
      summary: {
        ...
        average_order_value:
          totalOrders > 0 ? parseFloat((totalPointsForAvg / totalOrders).toFixed(2)) : 0,
