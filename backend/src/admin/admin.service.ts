...
export class AdminService {
  ...
  async getOrderAnalytics() {
    // Optimization: Consolidate 6 database queries into 2.
    // Query 1: Get total count and revenue grouped by status
    const statsByStatus = await this.ordersRepository
    ...
    // Query 2: Get orders by day (last 7 days)
    ...
    // Derive summary metrics in-memory from Query 1
    let totalOrders = 0;
    let totalRevenue = 0;
    ...
    // Derive recent orders count in-memory from Query 2
    ...
    return {
      summary: {
        ...
        average_order_value: totalOrders > 0 ? totalAmountAllOrders / totalOrders : 0,
