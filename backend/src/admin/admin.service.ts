...
export class AdminService {
  ...
  async getOrderAnalytics() {
    // 1. Group by status to get total orders, revenue, avg value, and breakdown in 1 roundtrip
    ...
    statusStats.forEach((item) => {
      ...
      totalOrders += count;
      grandTotal += sum;
      status_breakdown[item.status] = count;
      if (['shipped', 'delivered'].includes(item.status)) {
        totalRevenue += sum;
      }
    });

    // 2. Group by day for the last 7 days to get recent count and breakdown in 1 roundtrip
    ...
    const recentOrdersCount = dailyStats.reduce((acc, item) => acc + parseInt(item.count, 10), 0);

    return {
      summary: {
        total_orders: totalOrders,
        revenue: totalRevenue,
        average_order_value: totalOrders > 0 ? parseFloat((grandTotal / totalOrders).toFixed(2)) : 0,
