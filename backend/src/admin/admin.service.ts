...
export class AdminService {
  ...
  async getOrderAnalytics() {
    // Optimization: Consolidate 6 individual queries into 2 grouped queries to reduce DB roundtrips.

    // 1. Get status-based metrics (Total Count, Status Breakdown, Revenue, Avg Value)
    ...
    statusStats.forEach((item) => {
      ...
      totalOrders += count;
      totalSumForAllOrders += sum;
      statusBreakdown[status] = count;

      // Revenue defined as total amount from completed/shipped orders
      if (status === 'shipped' || status === 'delivered') {
        totalRevenue += sum;
      }
    });

    // 2. Get date-based metrics (Last 7 days)
    ...
    return {
      summary: {
        ...
        average_order_value: totalOrders > 0 ? totalSumForAllOrders / totalOrders : 0,
