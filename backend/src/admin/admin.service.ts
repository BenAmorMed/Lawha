...
export class AdminService {
  ...
  async getOrderAnalytics() {
    // Optimization: Consolidate 6 database roundtrips into exactly 2 by deriving
    // summary metrics in-memory from grouped aggregate results.

    // 1. Status-based stats (provides total, revenue, AOV, and breakdown)
    ...
    // 2. Date-based stats (provides recent count and daily breakdown)
    ...
    // Process status results in-memory
    let totalOrders = 0;
    let revenue = 0;
    let totalValue = 0;
    ...
    statusStats.forEach((item) => {
      ...
      // Revenue includes only completed (shipped/delivered) orders
      if (['shipped', 'delivered'].includes(status)) {
        revenue += sum;
      }
    });

    // Process date results in-memory
    const orders_last_7_days = dailyStats.reduce(
    ...
    return {
      summary: {
        ...
        average_order_value:
          totalOrders > 0 ? parseFloat((totalValue / totalOrders).toFixed(2)) : 0,
        orders_last_7_days,
      },
      status_breakdown,
      orders_by_day: dailyStats.map((item) => ({
        date: item.date,
