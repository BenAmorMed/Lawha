...
export class AdminService {
  ...
  async getOrderAnalytics() {
    ...
    // Optimization: Consolidate multiple analytics queries into two optimized ones
    // and execute them in parallel to reduce database roundtrips and overall latency.
    const [statusStats, dailyStats] = await Promise.all([
      // Query 1: Aggregated status statistics, revenue, and recent counts
      ...
      // Query 2: Daily counts for the last 7 days
      this.ordersRepository
        .createQueryBuilder('order')
    ...
    // In-memory derivation of global metrics from consolidated results
    let totalOrders = 0;
    let totalRevenue = 0;
    let totalPoints = 0; // for average calculation
