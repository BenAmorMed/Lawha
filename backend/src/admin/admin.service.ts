...
export class AdminService {
  ...
  async getOrderAnalytics() {
    // Optimization: Consolidate multiple individual queries into 2 grouped queries
    // and perform the remaining calculations in-memory.

    // 1. Get stats grouped by status for ALL orders
    // This replaces 4 individual queries: total count, status breakdown, revenue, and average order value.
    const statusStats = await this.ordersRepository
      .createQueryBuilder('order')
      .select('order.status', 'status')
      .addSelect('COUNT(order.id)', 'count')
      .addSelect('SUM(order.total)', 'sum_total')
    ...
    statusStats.forEach((item) => {
      ...
      const sum = parseFloat(item.sum_total || 0);
      const status = item.status;

      totalOrders += count;
      totalAmountAllOrders += sum;
      status_breakdown[status] = count;

      // Completed statuses for revenue
      if (['shipped', 'delivered'].includes(status)) {
        totalRevenue += sum;
      }
    });

    // 2. Get stats grouped by day for the last 7 days
    // This replaces 2 queries: recent orders count and orders by day distribution.
    ...
    const orders_last_7_days = ordersByDay.reduce(
    ...
    return {
      summary: {
        ...
        average_order_value:
          totalOrders > 0 ? totalAmountAllOrders / totalOrders : 0,
