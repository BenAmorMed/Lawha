...
export class AdminService {
  ...
  async getOrderAnalytics() {
    // ⚡ Optimization: Consolidate multiple queries into 2 database roundtrips
    // Query 1: Get global stats and status breakdown in one go
    ...
    statusStats.forEach((item) => {
      ...
      // Revenue is defined as total from completed orders
    ...
    // Query 2: Get daily stats for the last 7 days
    ...
    // Derive recent orders count from the daily stats to save a query
    const recentOrders = ordersByDayRaw.reduce(
