...
export class AdminService {
  ...
  async getAllOrders(filters: {
    ...
    // Optimization: Use getManyAndCount to get both data and total in one query.
    // Use loadRelationCountAndMap to fetch only the count of items instead of joining all item rows.
  ...
  async getOrderAnalytics() {
    ...
    // Optimization: Consolidate 6 sequential database queries into 2 parallel ones.
    // The first query fetches status-based counts, revenue, and recent order counts in a single pass.
    // The second query fetches the time-series data for the last 7 days.
    const [statsByStatus, ordersByDay] = await Promise.all([
      this.ordersRepository
        .createQueryBuilder('order')
        .select('order.status', 'status')
        .addSelect('COUNT(order.id)', 'count')
        .addSelect('SUM(order.total)', 'status_revenue')
    ...
    statsByStatus.forEach((item) => {
      ...
      const statusRevenue = parseFloat(item.status_revenue || 0);
      const recentCount = parseInt(item.recent_count || 0, 10);
      const status = item.status;

      totalOrders += count;
      totalValueSum += statusRevenue;
      ordersLast7Days += recentCount;
      statusBreakdown[status] = count;

      // Revenue only includes 'shipped' and 'delivered' orders
      if (['shipped', 'delivered'].includes(status)) {
        totalRevenue += statusRevenue;
      }
    });
    ...
    return {
      summary: {
        ...
        average_order_value: totalOrders > 0 ? totalValueSum / totalOrders : 0,
