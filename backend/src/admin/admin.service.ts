...
export class AdminService {
  ...
  async getAllOrders(filters: {
    ...
    // Optimization: Use loadRelationCountAndMap to avoid loading full OrderItem entities (large designJson).
    // Also use getManyAndCount to retrieve data and total count in one database roundtrip.
  ...
  async getOrderAnalytics() {
    // Optimization: Reduce database roundtrips from 6 to 2.
    // 1. Consolidated query for global metrics grouped by status.
    const globalStats = await this.ordersRepository
    ...
    // 2. Consolidated query for 7-day stats.
    ...
    const ordersLast7Days = ordersByDay.reduce((acc, item) => acc + parseInt(item.count, 10), 0);

    return {
      summary: {
        total_orders: totalOrders,
        revenue: parseFloat(revenue.toFixed(2)),
        average_order_value: totalOrders > 0 ? parseFloat((totalSum / totalOrders).toFixed(2)) : 0,
