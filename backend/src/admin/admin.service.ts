...
export class AdminService {
  ...
  async getAllOrders(filters: {
    ...
    // Optimization: Use loadRelationCountAndMap for items count instead of leftJoinAndSelect.
    // This avoids fetching the full OrderItem entities (and their large designJson) for the list view.
  ...
  async getOrderAnalytics() {
    // Optimization: Consolidate metrics (total count, status breakdown, revenue, and average value)
    // into ONE grouped query instead of 4 separate calls.
    ...
    const avgOrderValue = totalOrders > 0 ? totalValueSum / totalOrders : 0;

    // Optimization: Consolidate 7-day metrics into ONE grouped query.
    ...
    const ordersLast7Days = ordersByDayRaw.reduce((sum, item) => sum + parseInt(item.count, 10), 0);
    ...
    return {
      ...
      orders_by_day: ordersByDayRaw.map((item) => ({
        date: item.date,
