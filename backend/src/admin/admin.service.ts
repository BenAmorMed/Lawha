...
export class AdminService {
  ...
  async getAllOrders(filters: {
    ...
    // Optimization: Use loadRelationCountAndMap to fetch item count without loading full OrderItem entities
    // which can contain large designJson blobs, significantly reducing memory usage.
  ...
  async getOrderAnalytics() {
    ...
    // Query 1: Get summary metrics and status breakdown in one grouped query
    ...
    statusStats.forEach(stat => {
      ...
      const sum = parseFloat(stat.sumTotal || 0);
    ...
    // Query 2: Get recent orders and daily breakdown in one grouped query
    ...
    const ordersLast7Days = ordersByDay.reduce((sum, item) => sum + parseInt(item.count, 10), 0);
