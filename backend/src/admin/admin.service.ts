...
export class AdminService {
  ...
  async getAllOrders(filters: {
    ...
    // Whitelist sortBy fields to prevent SQL injection and ensure consistency
  ...
  async getOrderAnalytics() {
    // Consolidated Query 1: Fetch metrics grouped by status
    ...
    statusMetrics.forEach(item => {
      ...
      const avg = parseFloat(item.averageValue || 0);

      totalOrders += count;
      statusBreakdown[item.status] = count;

      // Revenue only from shipped/delivered orders
      if (['shipped', 'delivered'].includes(item.status)) {
        totalRevenue += revenue;
      }

      // Track total sum to calculate weighted average later
      totalValueSum += revenue;
    });

    // Consolidated Query 2: Fetch daily counts for the last 7 days
    ...
    const ordersLast7Days = dailyStats.reduce((sum, item) => sum + parseInt(item.count, 10), 0);
