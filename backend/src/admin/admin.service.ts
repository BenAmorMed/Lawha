...
export class AdminService {
  ...
  async getOrderAnalytics() {
     ...
     * We use status-based grouping to calculate overall totals, revenue, and averages in one go,
     * and a single date-based query for both the 7-day trend and the total recent orders count.
     */

    // Query 1: Get status-grouped counts and totals for all orders
    ...
    statusStats.forEach((item) => {
      ...
      const sum = parseFloat(item.totalSum || 0);
      const status = item.status;

      totalOrders += count;
      totalAmountForAll += sum;
    ...
    // Query 2: Get daily counts for the last 7 days
    ...
    const ordersLast7Days = dailyStats.reduce(
    ...
    return {
      summary: {
        ...
        average_order_value: totalOrders > 0 ? parseFloat((totalAmountForAll / totalOrders).toFixed(2)) : 0,
