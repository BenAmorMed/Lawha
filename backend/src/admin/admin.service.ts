...
export class AdminService {
  ...
  async getOrderAnalytics() {
    ...
    // Query 1: Metrics by status (total count, sum of total)
    ...
    // Query 2: Daily distribution (last 7 days)
    ...
    // Calculate recent orders count in-memory from daily distribution
    const recentOrdersCount = ordersByDay.reduce((acc, item) => acc + parseInt(item.count, 10), 0);

    return {
      summary: {
        total_orders: totalOrders,
        revenue: totalRevenue,
        average_order_value: totalOrders > 0 ? overallTotalSum / totalOrders : 0,
