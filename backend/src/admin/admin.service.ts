...
export class AdminService {
  ...
  async getOrderAnalytics() {
    // Optimization: Consolidate 6 database queries into 2 to reduce roundtrips and database load.

    // 1. Get stats grouped by status to derive total orders, status breakdown, revenue, and average value.
    ...
    // 2. Get daily stats for the last 7 days to derive recent count and daily breakdown.
    ...
    return {
      summary: {
        ...
        average_order_value: totalOrders > 0 ? parseFloat((totalValue / totalOrders).toFixed(2)) : 0,
