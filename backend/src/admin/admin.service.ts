...
export class AdminService {
  ...
  async getOrderAnalytics() {
    // Optimization: Reduce database roundtrips from 6 to 2 by consolidating aggregate queries.
    // First query: Get all metrics grouped by status (total count and sum for each status)
    ...
    // Second query: Get orders by day for the last 7 days
    ...
    return {
      summary: {
        ...
        average_order_value:
          totalOrders > 0
            ? parseFloat((allOrdersTotalAmount / totalOrders).toFixed(2))
            : 0,
