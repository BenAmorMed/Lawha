...
export class AdminService {
  ...
  async getAllOrders(filters: {
    ...
    // Use loadRelationCountAndMap to get items count efficiently.
  ...
  async getOrderAnalytics() {
    // Optimization: Group queries using Promise.all and reduce database roundtrips.
    // Calculate total orders and status breakdown in a single query by status.
    const sevenDaysAgo = new Date();
    sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);

    const [statusBreakdown, aggregateStats, ordersByDay] = await Promise.all([
      // 1. All-time orders by status
      ...
      // 2. Revenue (shipped/delivered) and Average Order Value
      this.ordersRepository
        .createQueryBuilder('order')
        .select(
    ...
    const totalOrders = statusBreakdown.reduce((sum, item) => sum + parseInt(item.count, 10), 0);
    const recentOrders = ordersByDay.reduce((sum, item) => sum + parseInt(item.count, 10), 0);

    return {
      summary: {
        total_orders: totalOrders,
        revenue: parseFloat(aggregateStats?.revenue || 0),
        average_order_value: parseFloat(aggregateStats?.avgValue || 0),
  ...
  async getAllReviews(filters: {
    ...
    // Optimization: Use getManyAndCount() to get data and total in one query.
