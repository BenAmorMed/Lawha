...
export class OrdersService {
  ...
  async createOrder(dto: CreateOrderDto, userId?: string): Promise<OrderCreatedResponseDto> {
    ...
    // Bolt: Optimized by fetching ProductSize and its associated Product in a single query
    // and parallelizing it with the FrameOption lookup using Promise.all.
    const lookups: [Promise<ProductSize>, Promise<FrameOption | null>] = [
    ...
    const [size, frame] = await Promise.all(lookups);
